...
async function init(options = {}) {
  ...
  try {
    ...
    if (options.force && frameworkExists) {
      if (options.verbose) {
        console.log(chalk.yellow('🔄 Force mode: Removing existing framework...'));
      } else {
        spinner.text = 'Force mode: Removing existing framework...';
      }
      await FileManager.removeFramework(currentDir);
    }
