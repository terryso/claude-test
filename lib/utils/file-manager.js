...
class FileManager {
  ...
  static async copyFrameworkFiles(targetDir, options = {}) {
    // Use the template directory from the CLI package
    const templateDir = this._getTemplateDir();
    const claudeDir = path.join(targetDir, '.claude');
  ...
  static async _copyWithProgress(src, dest, options = {}) {
    ...
    if (await fs.pathExists(commandsSrc)) {
      ...
      if (options.verbose && process.env.NODE_ENV !== 'test') {
        ...
        const files = await fs.readdir(commandsSrc);
    ...
    // Skip settings.local.json - it's local configuration, not a template
  }
   ...
   * @private
   */
  static _getTemplateDir() {
    // Use the templates directory in the CLI package
